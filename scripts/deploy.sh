...
echo "=========================================="
echo "Starting Crypto Payments deployment for branch: $BRANCH"
echo "=========================================="
...
$GO_CMD build -v -ldflags="-X main.appVersion=$(git describe --tags --long --dirty)" -o crypto-payments-server main.go
if [ $? -ne 0 ]; then
    echo "Build failed!"
    exit 1
fi
...
if systemctl cat $SERVICE_NAME > /dev/null 2>&1; then
    ...
    chmod +x "$PROJECT_DIR/scripts/create_service.sh"
    "$PROJECT_DIR/scripts/create_service.sh"
fi
